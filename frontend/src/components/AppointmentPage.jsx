...
const AppointmentPage = () => {
  ...
  const loadDoctorAppointments = useCallback(async () => {
    ...
    console.log(
      "Outgoing headers for /api/appointments/me:",
      headers,
    );

    try {
      const resp = await API.get("/api/appointments/me", {
        headers,
      });

      console.log(
        "Response from /api/appointments/me:",
        resp?.data,
      );
      ...
      console.log("DOCTOR APPOINTMENTS FROM API:", arr);
      console.log("DOCTOR APPOINTMENT COUNT:", arr.length);
      ...
      console.log("FILTERED DOCTOR APPOINTMENTS:", doctors);
      console.log("FILTERED DOCTOR COUNT:", doctors.length);

      setDoctorAppts(doctors);

      setAppointmentsRaw((p) => ({
        ...p,
        doctors: doctors,
      }));
    } catch (err) {
      ...
      if (user?.id) {
        try {
          console.log(
            "Attempting debug request with ?createdBy=",
            user.id,
          );
  ...
  const loadServiceAppointments = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "Failed to get Clerk token (frontend): err",
        err,
      );
    }
    ...
    try {
      ...
      console.log(
        "Response from /api/service-appointments/me:",
        resp?.data,
      );
      ...
      if (user?.id) {
        try {
          ...
          console.log(
            "Debug fallback response (services):",
            debugResp?.data,
          );
  ...
  const appointmentData = useMemo(() => {
    return doctorAppts
      .map((a) => {
        ...
        const doctorName =
          (doctorObj.name &&
            String(doctorObj.name).trim()) ||
          (a.doctorName &&
            String(a.doctorName).trim()) ||
          (a.doctor &&
            String(a.doctor).trim()) ||
          (a.patientName &&
  ...
  return (
    <div className={appointmentPageStyles.pageContainer}>
      ...
      <div
        className={
          appointmentPageStyles.maxWidthContainer
        }
      >
        <h1
          className={
            appointmentPageStyles.doctorTitle
          }
        >
        ...
        {loadingDoctors && (
          <div
            className={
              appointmentPageStyles.loadingText
            }
          >
            Loading Doctors...
          </div>
        ...
        {!loadingDoctors &&
          appointmentData.length === 0 && (
            <div
              className={
                appointmentPageStyles.emptyStateText
              }
            >
        ...
        <div
          className={
            appointmentPageStyles.doctorGrid
          }
        >
          {appointmentData.map((item) => (
            <div
              key={item.id}
              className={cardStyles.doctorCard}
            >
              <div
                className={
                  cardStyles.doctorImageContainer
                }
              >
                <img
                  src={
              ...
              <h2
                className={cardStyles.doctorName}
              >
                {item.doctor}
              </h2>
              ...
              <p
                className={
                  cardStyles.dateContainer
                }
              >
                <CalendarDays
                  className={iconSize.medium}
                />
                {item.date}
              </p>
              ...
              item.rescheduledTo ? (
                <div
                  className={
                    cardStyles.rescheduledText
                  }
                >
                  Reschedule to{" "}
                  <span
                    className={
                      cardStyles.rescheduledSpan
                    }
                  >
                    {item.rescheduledTo.date} :
                    {item.rescheduledTo.time}
        ...
        <h2
          className={
            appointmentPageStyles.serviceTitle
          }
        >
        ...
        {loadingServices && (
          <div
            className={
              appointmentPageStyles.serviceLoadingText
            }
          >
        ...
        {!loadingServices &&
          serviceData.length === 0 && (
            <div
              className={
                appointmentPageStyles.serviceEmptyStateText
              }
            >
        ...
        <div
          className={
            appointmentPageStyles.serviceGrid
          }
        >
          {serviceData.map((srv) => (
            <div
              key={srv.id}
              className={cardStyles.serviceCard}
            >
              <div
                className={
                  cardStyles.serviceImageContainer
                }
              >
                <img
                  src={
              ...
              <h3
                className={cardStyles.serviceName}
              >
                {srv.name}
              </h3>
              ...
              <p
                className={
                  cardStyles.serviceDateContainer
                }
              >
                <CalendarDays
                  className={iconSize.medium}
              ...
              <p
                className={
                  cardStyles.serviceTimeContainer
                }
              >
                <Clock
                  className={iconSize.medium}
              ...
              >
                ...
                <StatusBadge
                  itemStatus={srv.status}
                />
              </div>

              {srv.status === "Rescheduled" &&
              srv.rescheduledTo ? (
                <div
                  className={
                    cardStyles.serviceRescheduledText
                  }
                >
                  Rescheduled to{" "}
                  <span
                  ...
                  >
                    {srv.rescheduledTo.date} :{" "}
                    {srv.rescheduledTo.time}
                  </span>
                </div>
              ) : null}
            </div>
