...
// import { updateAppointment } from "./appointmentController";
...
//to getServiceAppointments By Id
export const getServiceAppointmentById = async (req, res) => {
  try {
    ...
    if (!appt)
      return res.status(404).json({
        success: false,
        message: "not found this appointment",
      });
...
export const updateServiceAppointment = async (req, res) => {
  try {
    ...
    //first check wherther fill if yes then updated
...
//to cancel the serviceAppointement 
...
export const getServiceAppointmentStats = async (req, res) => {
  try {
    ...
    // IMPORTANT: response send karna zaroori hai
